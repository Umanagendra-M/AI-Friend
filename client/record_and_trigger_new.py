...
class VoiceRecorder:
    ...
    def stop_recording(self):
        ...
        try:
            ...
            logger.exception ("Exception during stream.stop_stream")

        try:
            self.audio.terminate()
        except Exception as e:
            logger.exception ("Exception during audio.terminate")
        ...
        try:
            ...
            logger.info (f"Saved to {self.temp_wav_file}")
            return self.temp_wav_file # Return the filename
        except Exception as e:
            logger.exception(f"error during closing audio")
            return None
    ...
    def process_audio(self, model="llama2"):
        ...
        prompt = f"Based on this transcription: {transcription_result.get('text', '')}, what is the main topic?"
            ...
            if llm_response is None:
                ...
                try:
                    ...
                    if tts_response.status_code == 200:
                        ...
                        self._play_audio_stream(audio_filename)
                    else:
    ...
    def _play_audio_stream(self, audio_filename):
        """Plays the audio from file.
        Args:
            audio_filename: Path to the wav file
        """
        try:
...
def main():
    ...
    # Record and process
    result = record_and_process(args)
